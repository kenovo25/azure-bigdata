...
def main():
  ...
  arm_resources.append(azureDataLakeStoreLinkedService(
     "AzureAdlsStoreLinkedService", datafactory_name, datalake_store_name, 
      client_id, secret, subscription_id, tenant, 
      datalake_store_resource_group_name))
  arm_resources.append(azureDataLakeAnalyticsLinkedService(
     "AzureDataLakeAnalyticsLinkedService", datafactory_name, 
     datalake_analytics_name, client_id, secret, subscription_id, tenant, 
     datalake_analytics_resource_group_name))
  arm_resources.append(azureSqlDatabase("AzureSqlLinkedService", 
     datafactory_name, sqlserver_username, sqlserver_name, sqlserver_catalog, sqlserver_password))
  arm_resources.append(azureSqlDatabase("AzureSqlLinkedServiceAsie", 
     datafactory_name, sqlserver_username, sqlserver_name, sqldatabase_as_name, sqlserver_password))
  arm_resources.append(azureStorageLinkedService("AzureStorageLinkedService", 
     datafactory_name, storage_account_name, storage_account_key))
  arm_resources.append(azureDataLakeStoreLinkedService(
     "Destination-DataLakeStore-50d", datafactory_name, datalake_store_name, 
     client_id, secret, subscription_id, tenant, 
     datalake_store_resource_group_name))
  arm_resources.append(azureStorageLinkedService("Source-BlobStorage-50d", 
     datafactory_name, storage_account_name, storage_account_key))
...
def azureSqlDatabase(linked_service_name, datafactory_name, sqlserver_username, sqlserver_name, sqlserver_catalog, sqlserver_password):
   return json.loads(
'''{
    "name": "''' + linked_service_name + '''",
    "apiVersion": "2015-10-01",
